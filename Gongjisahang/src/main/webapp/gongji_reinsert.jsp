...
<head> <!--머릿말 시작-->
	...
	<%	 //scriptlet시작
		...
		if(relevel == 0 && recnt == 0) { //원글에 직접 댓글이 달릴 때는
			recnt = gongjidao.getMaxRecnt(rootid); //원글의 마지막 순서에 댓글이 입력된다
		} else {	//그렇지 않을 때는
			recnt = gongjidao.getMiddleRecnt(rootid, relevel, recnt); //원글에서 해당 댓글이 입력될 중간순서를 구해주고
			if(recnt == 0) { //해당순서가 0으로 나오면 마지막 댓글에 달릴 댓글이라는 뜻이므로
				recnt = gongjidao.getMaxRecnt(rootid);	//원글의 마지막 순서에 댓글이 입력된다
			}
		}
