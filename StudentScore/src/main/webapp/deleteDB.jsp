...
<% //scriptlet 시작
    ...
    Integer sid=0; //정수형 변수 sid에 0 대입
    try{
        sid = Integer.parseInt(request.getParameter("sid"));
        //sid파라미터를 받아와 정수형변환 후 변수 sid에 대입 
	...
	studentScoreDao.delete(sid);
	//StudentScoreDaoImpl 클래스의 sid를 매개변수로 하는 delete 메서드 호출
	//특정 학생의 정보를 삭제하는 함수
...
%> <!-- scriptlet 끝 -->
    <tr> <!-- 행 시작 -->
	    <td colspan=8 style="border: none;"> <!--해당 행의 8열까지 합쳐주고 테두리는 없음-->
	        ...
	        %>
	                <a href="deleteDB.jsp?pageNo=<%=PP %>&cntPT=<%=cntPT %>"><<</a> 
	                <!-- <<버튼을 누르면 첫번째 페이지로 이동한다-->
	                <a href="deleteDB.jsp?pageNo=<%=P %>&cntPT=<%=cntPT %>"><</a>
        	...
        	%>
                        <a style="color: red;" href="deleteDB.jsp?pageNo=<%=i %>&cntPT=<%=cntPT %>"><%=i %></a>
                        <!--현재페이지 버튼번호를 빨간색으로 출력해준다-->
        	<%
                    } else {
        	%>
                        <a href="deleteDB.jsp?pageNo=<%=i %>&cntPT=<%=cntPT %>"><%=i %></a>
	        ...
	        %> 
	                <a href="deleteDB.jsp?pageNo=<%=N %>&cntPT=<%=cntPT %>">></a>
	                <!-- >버튼을 누르면 다음 블록으로 이동한다-->
	                <a href="deleteDB.jsp?pageNo=<%=NN%>&cntPT=<%=cntPT %>">>></a>
